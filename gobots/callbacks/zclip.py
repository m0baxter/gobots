...
def is_fsdp_model(pl_module):
    return isinstance(pl_module, FSDP) or any(
        isinstance(m, FSDP) for m in pl_module.modules()
    )
...
class ZClip:
    ...
    def _update_ema(self, grad_norm):
        ...
        self.var = (
            self.alpha * self.var + (1 - self.alpha) * (grad_norm - self.mean) ** 2
        )
        ...
        if self.mode == "percentile":
            ...
            if z > self.z_thresh:
                if self.clip_option == "adaptive_scaling":
                    eta = (
                        z / self.z_thresh
                    )  # This rescaling ratio imposes a greater penalty on large outliers.
    ...
    def apply_in_place_clipping(
        self, pl_module, global_norm: float, max_global_norm: float
    ):
        """
        ...
        clip_coef = (
            (max_global_norm / (global_norm + 1e-6))
                ...
                if isinstance(model, FSDP):
                    ...
                    for m in model.modules():
                        if isinstance(m, FSDP) and m._is_root:
                            ...
                            grads_clipped = True
                    assert grads_clipped, (
                        "At least one root FSDP module must be available for gradient clipping when requested."
                    )
